...
export async function updateEventAction(eventId: string, formData: FormData) {
  ...
  try {
    ...
    const { data: hostProfile, error: profileError } = await supabase.from('host_pages')
      .select('user_id')
...
export async function uploadImageAction(
  ...
  if (id !== 'new-event' && id !== 'new-host' && !id.startsWith('temp-')) {
    ...
    const { data: hostProfile } = await supabase.from('host_pages').select('user_id').eq('id', event.host_id).single()
...
export async function deleteImageAction(url: string, eventId?: string) {
  ...
  if (eventId && eventId !== 'new-event' && !eventId.startsWith('temp-')) {
    ...
    if (event) {
      const { data: host } = await supabase.from('host_pages').select('user_id').eq('id', event.host_id).single()
...
export async function deleteEventAction(eventId: string) {
  ...
  const { data: hostProfile } = await supabase.from('host_pages')
    .select('user_id')
    .eq('id', event.host_id)
    .single()
...
export async function verifyEventFeePaymentAction(eventId: string, paymentDetails: { razorpay_order_id: string, razorpay_payment_id: string, razorpay_signature: string }) {
  ...
  const { error: updateError } = await adminSupabase.from('events')
   ...
   try {
     ...
     if (eventData) {
       const { data: hostProfile } = await adminSupabase.from('host_pages').select('user_id').eq('id', eventData.host_id).single()
       if (hostProfile) {
         ...
         if (hostUser?.email) {
           ...
           await fetch(functionUrl, {
             ...
             body: JSON.stringify({
               ...
               subject: 'Event Published Successfully! — City Culture',
...
export async function getOtherHostsAction() {
  ...
  const { data, error } = await supabase.from('host_pages').select('user_id, display_name, organisation_name').neq('user_id', user.id).order('display_name')
  if (error) return { error: error.message }
