...
const HOST_PRODUCTION_ORIGIN = 'https://host.cityculture.in'

/**
 * Origin used in Supabase auth redirectTo (password reset, OAuth).
 * Uses the incoming request host so recovery links stay on host.cityculture.in
...
export async function getAuthCallbackOrigin(): Promise<string> {
  ...
  if (siteUrl.includes('host.cityculture.in')) {
    return siteUrl
  }
